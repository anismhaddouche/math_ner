...
sys.path.append("../model/fr_LabnbookNer-0.0.0")
...
def visualize_labdoc(id_mission: int, id_report: int, id_labdoc: int):
    ...
    if st.button("Get a summary", key="summary"):
        ...
        st.text(
            f" \
                eqc membership: {fuzzified_mission[fuzzified_mission['id_labdoc'] == id_labdoc]['eqc_membership'].values[0]} \n \
                eqc degree : {fuzzified_mission[fuzzified_mission['id_labdoc'] == id_labdoc]['eqc_degree'].values[0]} \n \
                coec membership : {fuzzified_mission[fuzzified_mission['id_labdoc'] == id_labdoc]['coec_membership'].values[0]} \n \
                coec degree : {fuzzified_mission[fuzzified_mission['id_labdoc'] == id_labdoc]['coec_degree'].values[0]}"
        )
